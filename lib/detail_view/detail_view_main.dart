...
class DetailViewMain extends StatefulWidget {
  ...
  DetailViewMain(this.theme, this.restaurant, this.index,
      this.myFavoriteKey); // const DetailViewMain({Key? key}) : super(key: key);
...
class _DetailViewMainState extends State<DetailViewMain> {
  ...
  initFavorite() async {
    ...
    if (favoriteList != null) {
      if(favoriteList.contains(widget.restaurant.id!)) {
        setState(() {
          isFavorite = true;
        });
      }
          ...
          return CustomScrollView(
            ...
            slivers: [
              SliverAppBar(
                ...
                actions: <Widget>[
                  IconButton(
                    icon: Icon(isFavorite ? Icons.favorite : Icons.favorite_border, color: isShrink ? AppColor.primaryFill : AppColor.surface),
                    onPressed: () async {
                      setState(() {
                      ...
                      if (favoriteList == null) {
                        favoriteList = [];
                        if (isFavorite) {
                          favoriteList.add(widget.restaurant.id!);
                        }
                      }else {
                        if (isFavorite) {
                          favoriteList.add(widget.restaurant.id!);
                        }else {
                          favoriteList.removeWhere((element) => element == widget.restaurant.id!);
                        }
                      }
